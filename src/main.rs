...
fn main() {
    let matches = App::new("rtree")
        ...
        .arg(
            ...
            Arg::with_name("indent")
                .short("i")
                .long("indent")
                .help("show structure using 4 spaces indent"),
        ).arg(
            Arg::with_name("root")
                .value_name("ROOT")
                .short("r")
    ...
    match indent {
        true => print::print_file_node_simplely(&root_node, 1, level, human),
        false => print::print_file_node_tree(&root_node, &mut vec![], 1, level, human),
    }
}
